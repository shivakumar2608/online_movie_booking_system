...
<?php 
  ...
  if(isset($_POST['updateNewPassword'])){
    ...
    $duplicate = mysqli_query($conn, "SELECT * FROM users WHERE username = '$username' or email='$username'");
    if(mysqli_num_rows($duplicate)>0){
        if($password == $password2){
        $query = "UPDATE users SET password='$password'  WHERE username ='$username' or email='$username'";
...
if(isset($_POST['submit'])){
  ...
  $mobile = $_POST['mobile'];
  $result = mysqli_query($conn, "SELECT * FROM users WHERE username = '$usernameemail' OR email = '$usernameemail' and mobile = '$mobile'"  );
